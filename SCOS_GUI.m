...
handles.btn_startVideo.UserData.calcSCOS_Flag = false;
handles.btn_startVideo.UserData.scosData = nan(1,10000);
handles.btn_startVideo.UserData.scosTime = nan(1,10000);
handles.btn_startVideo.UserData.resetSCOS_Flag = true;
handles.btn_startVideo.UserData.calcSCOS_lastClock = 0;
handles.btn_startVideo.UserData.calcSCOS_beforeVideoStop_Flag = false;
...
handles.btn_startVideo.UserData.saveEachNframes = 200;
...
if isequal(hObject.String,'Start SCOS') % 
    ...
    handles.btn_startVideo.UserData.calcSCOS_Flag = true;
    handles.btn_startVideo.UserData.resetSCOS_clock = tic;
    ...
    if ~isempty( find(isnan(handles.btn_startVideo.UserData.scosTime),1) )  && ...
            find(isnan(handles.btn_startVideo.UserData.scosTime),1) > 1
        handles.btn_startVideo.UserData.calcSCOS_Flag = false;
        handles.btn_startVideo.UserData.calcSCOS_lastClock = handles.btn_startVideo.UserData.scosTime(find(isnan(handles.btn_startVideo.UserData.scosTime),1)-1)  ; 
    end
    %TBD
    handles.tgl_startVideo.UserData.calcSCOS_Flag = false;
    handles.tgl_startVideo.UserData.calcSCOS_lastClock = handles.btn_startVideo.UserData.scosTime(find(isnan(handles.tgl_startVideo.UserData.scosTime),1)-1)  ; 

    set(hObject,'String','Start SCOS');
    
    if ~isempty(handles.btn_startVideo.UserData.scosTime) && any(~isnan(handles.btn_startVideo.UserData.scosTime))
        scosData = handles.btn_startVideo.UserData.scosData;
        scosTime = handles.btn_startVideo.UserData.scosTime;
...
handles.btn_startVideo.UserData.src.ExposureTime = str2double(hObject.String)*1000; % should be in microseconds
% stop(handles.btn_startVideo.UserData.vid);
% start(handles.btn_startVideo.UserData.vid);
...
handles.btn_startVideo.UserData.src.Gain = str2double(hObject.String); 
...
if ~isvalid(handles.btn_startVideo.UserData.src)
    ...
    handles.btn_startVideo.UserData.src.TriggerDelay = triggerDelayNum; % should be in microseconds
end
...
function chk_externalTrigger_Callback(hObject, eventdata, handles)
    ...
    if ~isvalid(handles.btn_startVideo.UserData.src)
        ...
        if ~vid_was_valid
            ...
            vid = handles.btn_startVideo.UserData.vid;
        end
        src = getselectedsource(vid);
        ...
        if hObject.Value
            set( handles.btn_startVideo.UserData.src,'TriggerMode' , 'on'); 
            set( handles.btn_startVideo.UserData.src, 'AcquisitionFrameRateEnable','False');
        else
            set( handles.btn_startVideo.UserData.src ,'TriggerMode' ,'off' );
            set( handles.btn_startVideo.UserData.src, 'AcquisitionFrameRateEnable','True');  
            frameRate = get( handles.btn_startVideo.UserData.src,'AcquisitionFrameRate');
        end
    end   
...
if handles.chk_externalTrigger.Value % we are working with external trigger
    ...
    if isvalid(handles.btn_startVideo.UserData.src)
        handles.btn_startVideo.UserData.src.AcquisitionFrameRateEnable = 'True';
        handles.btn_startVideo.UserData.src.AcquisitionFrameRate = frameRateNum;
    else
        vid = videoinput("gentl", 1, "Mono8");
        src = getselectedsource(vid);
...
% hObject    handle to edt_nOfSeconds (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edt_nOfSeconds as text
%        str2double(get(hObject,'String')) returns contents of edt_nOfSeconds as a double
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if isfield(handles.btn_startVideo.UserData.vid,'vid')
    vid = handles.btn_startVideo.UserData.vid;
    stop(vid)
    delete(vid);
    clear(vid)
...
function btn_open_recording_Callback(hObject, eventdata, handles)
    ...
    % --- Executes on button press in btn_setSCOSRecName.
function btn_setSCOSRecName_Callback(hObject, eventdata, handles)
% hObject    handle to btn_setSCOSRecName (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%     handles.fig_SCOS_GUI.UserData.isVideoRunning = true;
    ...
    src = getselectedsource(vid);    
    handles.btn_startVideo.UserData.src = src;
    ...
    scos_ind = find(isnan(handles.btn_startVideo.UserData.scosTime),1);
    if isempty(scos_ind)
        scos_ind = numel(handles.btn_startVideo.UserData.scosTime) + 1;        
    end
    currXlim = get(handles.ax_scos,'XLim');
    
    if handles.btn_startVideo.UserData.calcSCOS_beforeVideoStop_Flag
        btn_start_Callback(handles.btn_start, eventdata, handles)
    end
    hObject.Value =1;
    while isvalid(handles.fig_SCOS_GUI) && hObject.Value && isvalid(handles.btn_startVideo.UserData.src) && isvalid(vid) && vid.FramesAvailable  % VIKA TBD! 
...
%                timeJump = 0.5 ; %  min
           ...
           if scos_ind > length(handles.btn_startVideo.UserData.scosData)
           ...
           im = double(im);
           windowSize = str2double(handles.edt_windowSize.String);
           gain_dB = handles.btn_startVideo.UserData.src.Gain;
...
%                 vid = videoinput("gentl", 1, "Mono8");
%                 vid.FramesPerTrigger = Inf;
%                 handles.btn_startVideo.UserData.vid = vid;
%                 src = getselectedsource(vid); 
%                 handles.btn_startVideo.UserData.src = src;
%                 
%                 if strcmpi(src.TriggerMode,'On') %handles.chk_externalTrigger.Value
...
%                 start(vid)
           ...
           if mod(scos_ind, handles.btn_startVideo.UserData.saveEachNframes ) == 0 
               scosData = handles.btn_startVideo.UserData.scosData;
               scosTime = handles.btn_startVideo.UserData.scosTime;
               save(handles.fig_SCOS_GUI.UserData.recName,'scosData','scosTime');
           end
           ...
           %handles.btn_startVideo.UserData.scosData(scos_ind) =  mean( (stdIm(mask)- actualGain*meanIm(mask) - readoutN -1/12)./meanIm(mask) )^2  ;
           tm = toc(hObject.UserData.resetSCOS_clock) ;

           hObject.UserData.scosTime(scos_ind) =  tm/60 + handles.btn_startVideo.UserData.calcSCOS_lastClock  ; % /60 in order to convert to [min] from [sec]
...
%                 set(handles.ax_scos,'XLim',[0 timeJump*( mod(handles.btn_startVideo.UserData.scosTime(scos_ind),timeJump) + 1) ]); 
